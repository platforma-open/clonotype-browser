import type { AnnotationSpec, AnnotationSpecUi, BlockArgs, TableInputs } from '@platforma-open/milaboratories.clonotype-browser-3.model';
import { convertFilterSpecsToExpressionSpecs, type Option, plRefsEqual } from '@platforma-sdk/model';
...
export function syncDatasetTitle(
  ...
  watch(() => [getArgs().inputAnchor, getInputOptions()] as const, ([inputAnchor, inputOptions]) => {
...
export function syncTableInputs(
  ...
  watch(() => [getTableInputs(), getArgs().inputAnchor] as const, ([tableInputs, inputAnchor]) => {
