...
async function setupProject({
  ...
  const clonotypingStableState1Outputs = wrapOutputs<MiXCRClonotypingBlockOutputs>(
    clonotypingStableState1.outputs,
  );
  ...
  const clonotypingStableState3 = await helpers.awaitBlockDoneAndGetStableBlockState<
    typeof mixcrPlatforma
  >(clonotypingBlockId, 200000);
...
function findColumnId(
  columns:
    ...
    | undefined,
  labelOrPredicate: string | ((label: string) => boolean),
): SUniversalPColumnId | undefined {
  if (!columns) return undefined;
  const predicate =
    typeof labelOrPredicate === "string"
      ? (label: string) => label === labelOrPredicate
      : labelOrPredicate;
...
blockTest(
  ...
  async ({ rawPrj: project, ml, helpers, expect }) => {
    const { annotationBlockId, outputs4 } = await setupProject({
      rawPrj: project,
      ml,
      helpers,
      expect,
    ...
    const readCount652Column = findColumnId(outputs5.overlapColumns?.columns, (l) =>
      l.includes("Number Of Reads / SRR11233652"),
    );
    const readFraction652Column = findColumnId(outputs5.overlapColumns?.columns, (l) =>
      l.includes("Fraction of reads / SRR11233652"),
    );
    const readFraction664Column = findColumnId(outputs5.overlapColumns?.columns, (l) =>
      l.includes("Fraction of reads / SRR11233664"),
    );
    ...
    const columnSpecs = await ml.driverKit.pFrameDriver.getSpec(
      outputs6.overlapTable!.fullTableHandle,
    );
    // console.dir({ byClonotypeColSpecs: columnSpecs }, { depth: 8 });
    const annotationIdx = columnSpecs.findIndex(
      (col) => col.spec.name === "pl7.app/vdj/annotation",
    );
    expect(annotationIdx, "Annotation Column Index").toBeGreaterThanOrEqual(0);

    const annotationData = await ml.driverKit.pFrameDriver.getData(
      outputs6.overlapTable!.fullTableHandle,
      [annotationIdx],
    );
    ...
    expect(
      annotationData[0].data.some((val) => Boolean(val?.toString()?.includes("Medium Read Count"))),
      "Annotation contains label",
    ).toBe(true);

    const statsShape = await ml.driverKit.pFrameDriver.getShape(
      outputs6.statsTable!.fullTableHandle,
    );
    const statsData = await ml.driverKit.pFrameDriver.getData(
      outputs6.statsTable!.fullTableHandle,
      [...Array(statsShape.columns).keys()],
    );
