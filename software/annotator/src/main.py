...
class ValueRank(Base, tag='rank', tag_field="transformer"):
    ...
    def generate_expression(self, lf: pl.LazyFrame, group_by_col: Optional[str], unique_key_col: str) -> pl.Expr:
        ...
        stable_rank_col_expr = pl.struct(self.column, unique_key_col).rank(method='ordinal',
                                                                           descending=self.descending or False)
        ...
        if group_by_col:
            ...
            return stable_rank_col_expr  # Simpler for global case, relying on Polars default stability if any.
...
class SortedCumulativeSum(Base, tag='sortedCumulativeSum', tag_field="transformer"):
    ...
    def generate_expression(self, lf: pl.LazyFrame, group_by_col: Optional[str], unique_key_col: str) -> pl.Expr:
        ...
        if group_by_col:
            ...
            return pl.col(self.column).sort_by(sort_by_cols, descending=self.descending or False).over(
                group_by_col).cum_sum()
...
class NumericalComparisonFilter(Base, tag='numericalComparison'):
    ...
    def get_required_transformed_columns(self) -> Set[TransformedColumn]:
        cols = set()
        if isinstance(self.lhs, Base) and isinstance(self.lhs, (ValueRank, SortedCumulativeSum, Log10)): cols.add(
            self.lhs)
        if isinstance(self.rhs, Base) and isinstance(self.rhs, (ValueRank, SortedCumulativeSum, Log10)): cols.add(
            self.rhs)
        return cols
...
def validate_script_and_inputs(
    ...
    if script.mode == 'bySampleAndClonotype':
        if not script.sampleKeyColumn:
            print(f"Error: 'sampleKeyColumn' must be specified in the script when mode is 'bySampleAndClonotype'.",
                  file=sys.stderr)
            sys.exit(1)
        # Crucial: sample-clonotype data is MANDATORY in this mode
        if not sample_clonotype_data_path:
            print(
                f"Error: Sample-clonotype data file (--sample-clonotype-data) must be provided when mode is 'bySampleAndClonotype'.",
                file=sys.stderr)
            sys.exit(1)
        ...
        if not clonotype_data_path:
            print(f"Error: Clonotype data file (--clonotype-data) must be provided when mode is 'byClonotype'.",
                  file=sys.stderr)
            sys.exit(1)
        if script.sampleKeyColumn:
            print(
                f"Warning: 'sampleKeyColumn' ('{script.sampleKeyColumn}') is specified in the script but mode is 'byClonotype'. It will be ignored.",
                file=sys.stderr)
        if sample_clonotype_data_path:
            print(
                f"Warning: Sample-clonotype data file ('{sample_clonotype_data_path}') is provided but mode is 'byClonotype'. It will be ignored.",
                file=sys.stderr)
        print("Mode 'byClonotype' selected.")
    else:
        # Should be caught by msgspec decoding, but belt-and-suspenders
        print(f"Error: Unknown annotation mode '{script.mode}'. Must be 'byClonotype' or 'bySampleAndClonotype'.",
              file=sys.stderr)
        sys.exit(1)
...
def validate_columns_exist(
        ...
        script: AnnotationScript  # Added to access key columns
):
    ...
    if clono_schema and sample_schema:
        # Identify potential join keys to ignore in the warning
        keys_to_ignore = {script.clonotypeKeyColumn}
        if script.sampleKeyColumn:
            keys_to_ignore.add(script.sampleKeyColumn)

        overlap = clono_keys.intersection(sample_keys)
        # Only warn if overlap contains columns *other* than the known keys
        warning_overlap = overlap - keys_to_ignore
        if warning_overlap:
            # Allow overlap, especially for key columns used in join.
            print(
                f"Warning: The following columns (excluding known keys like '{script.clonotypeKeyColumn}') appear in both clonotype and sample-clonotype schemas: {warning_overlap}. Ensure this is intended.",
                file=sys.stderr)
    ...
    if missing_cols:
        print(f"Error: The following columns required by the script are missing from the input schemas:",
              file=sys.stderr)
        ...
        if clonotype_data_path and clono_schema:
            ...
            print(
                f"  Clonotype data file provided ({os.path.basename(clonotype_data_path)}) but schema is missing or failed to load.",
                file=sys.stderr)
        if sample_clonotype_data_path and sample_schema:
            ...
            print(
                f"  Sample-clonotype data file provided ({os.path.basename(sample_clonotype_data_path)}) but schema is missing or failed to load.",
                file=sys.stderr)
...
def main(
    ...
    if not clonotype_data and not sample_clonotype_data:
        print("Error: At least one data input (--clonotype-data or --sample-clonotype-data) must be provided.",
              file=sys.stderr)
        return 1
    ...
    if sample_clonotype_schema_path and not sample_clonotype_data:
        print("Warning: --sample-clonotype-schema provided without --sample-clonotype-data. It will be ignored.",
              file=sys.stderr)
    ...
    try:
        ...
        print(f"Error: Failed to decode annotation script {script_path}. Invalid JSON or structure: {e}",
              file=sys.stderr)
        return 1
    except Exception as e:
    ...
    if script.mode == 'bySampleAndClonotype' and sample_clonotype_data and not sample_clonotype_schema_path:
        print(f"Internal Error: Sample schema missing despite sample data being present for bySampleAndClonotype mode.",
              file=sys.stderr)
        return 1
    ...
    if clono_schema and script.clonotypeKeyColumn not in clono_schema:
        print(
            f"Error: Clonotype key column '{script.clonotypeKeyColumn}' defined in script not found in clonotype schema '{clonotype_schema_path}'.",
            file=sys.stderr)
        return 1
    if sample_schema:
        if script.mode == 'bySampleAndClonotype':
            if script.sampleKeyColumn not in sample_schema:
                print(
                    f"Error: Sample key column '{script.sampleKeyColumn}' defined in script not found in sample schema '{sample_clonotype_schema_path}'.",
                    file=sys.stderr)
                return 1
            ...
            if not clono_schema and script.clonotypeKeyColumn not in sample_schema:
                print(
                    f"Error: Clonotype key column '{script.clonotypeKeyColumn}' defined in script not found in sample schema '{sample_clonotype_schema_path}' (and clonotype data was not provided).",
                    file=sys.stderr)
                return 1
    ...
    if sample_clonotype_data and sample_schema:  # Check schema too
        ...
        try:
            ...
            print(f"Error reading or scanning sample-clonotype data table {sample_clonotype_data}: {e}",
                  file=sys.stderr)
            return 1
    ...
    if clono_transforms and lf_clono_transformed is not None:
        ...
        # Use a dictionary to prevent duplicate output column names
        expressions_to_add_clono = {}
        for t_col in clono_transforms:
            t_name = get_transformed_column_name(t_col)
            if t_name in expressions_to_add_clono:
                print(
                    f"    - Warning: Duplicate clonotype transformation found for output name '{t_name}'. Using the last definition.")
            # Generate expression based on the *original* LazyFrame
            expr = t_col.generate_expression(lf_clono, group_by_col=None, unique_key_col=script.clonotypeKeyColumn)
            expressions_to_add_clono[t_name] = expr  # Store expression
            transformed_exprs[t_name] = pl.col(t_name)  # Store ref for later use
            print(f"    - Preparing {t_name} based on {t_col.get_source_column()}")

        # Create the list of aliased expressions from the dictionary
        transform_selects_clono = [expr.alias(name) for name, expr in expressions_to_add_clono.items()]

        if transform_selects_clono:
            # Apply transformations and materialize to resolve potential intermediate name conflicts
            print("    Applying and materializing clonotype transformations...")
            lf_clono_transformed = lf_clono.with_columns(transform_selects_clono).collect().lazy()
    ...
    if sample_transforms and lf_sample_transformed is not None and script.mode == 'bySampleAndClonotype':
        print(
            f"  Applying {len(sample_transforms)} transformations to sample-clonotype data (grouped by {script.sampleKeyColumn})...")
        # Use a dictionary to prevent duplicate output column names
        expressions_to_add_sample = {}
        for t_col in sample_transforms:
            t_name = get_transformed_column_name(t_col)
            if t_name in expressions_to_add_sample:
                print(
                    f"    - Warning: Duplicate sample transformation found for output name '{t_name}'. Using the last definition.")
            # Generate expression based on the *original* LazyFrame
            expr = t_col.generate_expression(lf_sample, group_by_col=script.sampleKeyColumn,
                                             unique_key_col=script.clonotypeKeyColumn)
            expressions_to_add_sample[t_name] = expr  # Store expression
            transformed_exprs[t_name] = pl.col(t_name)  # Store ref for later use
            print(f"    - Preparing {t_name} based on {t_col.get_source_column()}")

        # Create the list of aliased expressions from the dictionary
        transform_selects_sample = [expr.alias(name) for name, expr in expressions_to_add_sample.items()]

        if transform_selects_sample:
            # Apply transformations and materialize to resolve potential intermediate name conflicts
            print("    Applying and materializing sample-clonotype transformations...")
            # Ensure we apply to the correct base frame (lf_sample)
            lf_sample_transformed = lf_sample.with_columns(transform_selects_sample).collect().lazy()
    ...
    if script.mode == 'bySampleAndClonotype':
        ...
        if lf is None:  # Should have been caught earlier
            print("Internal Error: Sample LazyFrame is missing in bySampleAndClonotype mode after transformations.",
                  file=sys.stderr)
            return 1
        ...
        if lf_clono_transformed is not None:
            ...
            clono_available_schema_names = set(
                lf_clono_transformed.collect_schema().names()) if lf_clono_transformed is not None else set()
            for src_col in required_source_cols:
                # Check against the schema names of the *potentially transformed* clono frame
                if src_col in clono_available_schema_names:
            ...
            # Determine columns already present in the left frame (lf, which is lf_sample_transformed)
            left_cols = set(lf.collect_schema().names())

            # Select columns from the right frame (clono) that are needed BUT NOT already in the left frame,
            # EXCLUDING the join key itself (handled by 'on=')
            clono_cols_to_select_final = list(
                (clono_join_cols_needed - {script.clonotypeKeyColumn}) - left_cols
            )
            # Ensure the selected columns actually exist in the (potentially transformed) clono frame
            clono_cols_to_select_final = [c for c in clono_cols_to_select_final if c in clono_available_schema_names]

            print(f"  Selecting columns from clonotype data for join: {clono_cols_to_select_final}")

            lf = lf.join(
                lf_clono_transformed.select([script.clonotypeKeyColumn] + clono_cols_to_select_final),
                # Select key + new columns
        ...
        if lf is None:  # Should have been caught earlier
            print("Internal Error: Clonotype LazyFrame is missing in byClonotype mode after transformations.",
                  file=sys.stderr)
            return 1
        # Performance warning fix: Use collect_schema()
    ...
    try:
        ...
        print(
            f"Error during final selection or writing: Column not found. This might indicate an issue with column naming or joining. {e}",
            file=sys.stderr)
        return 1
    except Exception as e:
